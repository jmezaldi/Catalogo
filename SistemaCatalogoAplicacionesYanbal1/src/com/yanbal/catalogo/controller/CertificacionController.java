...
import com.yanbal.catalogo.ejb.CertificacionFacade;
...
@ManagedBean(name = "certificacionController")
@SessionScoped
public class CertificacionController implements Serializable {

	private Certificacion current;
	private DataModel items = null;
	@EJB
	private com.yanbal.catalogo.ejb.CertificacionFacade ejbFacade;
	@EJB
	private com.yanbal.catalogo.ejb.UsuarioFacade ejbFacadeUsuario;
	private PaginationHelper pagination;
	private int selectedItemIndex;

	private List<Certificacion> filteredItems;
	...
	public Certificacion getSelected() {
		if (current == null) {
			current = new Certificacion();
			selectedItemIndex = -1;
		}
		return current;
	}

	private CertificacionFacade getFacade() {
		return ejbFacade;
	}
	...
	public String prepareView() {
		current = (Certificacion) getItems().getRowData();
	...
	public String create() {
		...
		try {
			...
			JsfUtil.addSuccessMessage(ResourceBundle.getBundle("/Bundle")
					.getString("CertificacionCreated"));
			util.sendEmailAccounts(email,ejbFacadeUsuario.findAll(), ResourceBundle.getBundle("/Bundle")
					.getString("ADD")
					+ current.getIdPlataformaCliente().getNombre()
					+ " - "
					+ current.getIdAplicacion().getNombre(), ResourceBundle
	...
	public String update() {
		...
		try {
			...
			JsfUtil.addSuccessMessage(ResourceBundle.getBundle("/Bundle")
					.getString("CertificacionUpdated"));
			util.sendEmailAccounts(email,ejbFacadeUsuario.findAll(), ResourceBundle.getBundle("/Bundle")
					.getString("EDIT")
					+ current.getIdPlataformaCliente().getNombre()
					+ " - "
	...
	private void performDestroy() {
		...
		try {
			...
			JsfUtil.addSuccessMessage(ResourceBundle.getBundle("/Bundle")
					.getString("CertificacionDeleted"));
			util.sendEmailAccounts(email,ejbFacadeUsuario.findAll(), ResourceBundle.getBundle("/Bundle")
					.getString("DELETE")
					+ current.getIdPlataformaCliente().getNombre()
					+ " - "
	...
	public DataModel getItems() {
		if (items == null) {
			...
			filteredItems = new ArrayList<Certificacion>(ejbFacade.findAll());
		}
		return items;
	}
	...
	@FacesConverter(forClass = Certificacion.class)
	public static class CertificacionControllerConverter implements Converter {
		...
		public Object getAsObject(FacesContext facesContext,
			...
			CertificacionController controller = (CertificacionController) facesContext
					.getApplication()
					.getELResolver()
		...
		public String getAsString(FacesContext facesContext,
			...
			if (object instanceof Certificacion) {
				Certificacion o = (Certificacion) object;
				return getStringKey(o.getId());
			} else {
				throw new IllegalArgumentException("object " + object
						+ " is of type " + object.getClass().getName()
						+ "; expected type: "
						+ CertificacionController.class.getName());
			}
		}
	}

	public String prepareEditCust(Certificacion obj) {
		current = obj;
		selectedItemIndex = pagination.getPageFirstItem()
				+ getItems().getRowIndex();
		return "Edit";
	...
	public String destroyCust(Certificacion obj) {
	...
	public List<Certificacion> getFilteredItems() {
		return filteredItems;
	}

	public void setFilteredItems(List<Certificacion> filteredItems) {
